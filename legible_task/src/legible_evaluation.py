...
OBJECTS_WORLDS = {1: '100x100_world_g3.yaml', 2: '100x100_world_g4.yaml', 3: '100x100_world_g5.yaml', 4: '100x100_world_g6.yaml',
				  5: '100x100_world_g7.yaml', 6: '100x100_world_g8.yaml', 7: '100x100_world_g9.yaml', 8: '100x100_world_g10.yaml'}
...
def world_evaluation(n_reps: int, beta: float, fail_prob: float, gamma: float, metric: str, world: str, miura_eval: Dict, policy_eval: Dict) -> None:
	...
	with open('../data/configs/' + world) as file:
	...
	for iteration in tqdm(range(n_reps), desc='Evaluation Repetitions'):
		...
		if metric == 'miura':
			...
			policy_performance = timeit.timeit(policy_stmt, number=10)
			miura_performance = timeit.timeit(miura_stmt, number=10)
...
def frameworks_evaluation(evaluation: str, n_reps: int, fail_prob: float, beta: float, gamma: float, metric: str) -> Tuple[Dict, Dict]:
	...
	miura_eval = manager.dict()
	policy_eval = manager.dict()
	if evaluation == 'scale':
	...
	for idx in worlds_keys:
		pool.apply_async(world_evaluation, args=(n_reps, beta, fail_prob, gamma, metric, worlds[idx], miura_eval, policy_eval))
	
	pool.close()
	pool.join()
	
	return dict(policy_eval), dict(miura_eval)
