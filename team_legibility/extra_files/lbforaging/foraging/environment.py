...
class ForagingEnv(Env):
    ...
    def _is_valid_action(self, player, action):
        if action == Action.NONE:
            ...
            return (
                player.position[0] > 0
                and self.field[max(0, player.position[0] - 1), player.position[1]] == 0
            )
        elif action == Action.SOUTH:
            return (
                player.position[0] < self.rows - 1
                and self.field[min(player.position[0] + 1, self.rows - 1), player.position[1]] == 0
            )
        elif action == Action.WEST:
            return (
                player.position[1] > 0
                and self.field[player.position[0], max(0, player.position[1] - 1)] == 0
            )
        elif action == Action.EAST:
            return (
                player.position[1] < self.cols - 1
                and self.field[player.position[0], min(self.cols - 1, player.position[1] + 1)] == 0
            )
